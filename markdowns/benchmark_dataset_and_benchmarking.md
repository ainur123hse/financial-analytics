...
- в `tp` перечисляются ключевые тезисы эталона, корректно восстановленные в candidate;
- в `fn` перечисляются ключевые тезисы эталона, отсутствующие в candidate или искаженные в нем;
- в `fp` перечисляются ключевые тезисы candidate, не подтверждаемые reference.
...
| `tp` | список ключевых тезисов эталона, корректно восстановленных в candidate |
| `fn` | список ключевых тезисов эталона, отсутствующих в candidate или искаженных в нем |
| `fp` | список ключевых тезисов candidate, не подтвержденных эталоном |
...
Файл `evaluation.json` хранит детализированный результат claim-level оценки, включая списки `tp`, `fn` и `fp`.
